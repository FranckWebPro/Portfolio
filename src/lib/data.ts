...
import { User, Stack, ProjectWithStacks, UserCredentials, Articles } from "./definitions";
...
export async function browseArticles(): Promise<Articles[]> {
  try {
    const articles = await sql<Articles>`
        SELECT
          id,
          created_at,
          title,
